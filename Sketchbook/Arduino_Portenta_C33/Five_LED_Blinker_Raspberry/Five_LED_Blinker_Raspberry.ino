...
void setup(void) {
  ...
  blink_led_raspi(LED_RASPI_HALT_PIN, 250, 5);
  delay(200);
  blink_led_raspi(LED_RASPI_WIFI_PIN, 250, 5);
  delay(200);
  blink_led_raspi(LED_RASPI_CONNECT_PIN, 250, 5);
...
void loop (void) {
  float potentiomenter_voltage;
  String text;
  uint8_t index = 0, looper = 0;
  uint16_t potentiomenter_reading;
  ...
  while(true) {
    ...
    potentiomenter_reading = analogRead(ANALOG_POT_PIN);
    potentiomenter_voltage = (MAXIMUM_ANALOG_VOLTAGE / ANALOG_RESOLUTION) * potentiomenter_reading; // * 1000.0;

    //  Print the potentiomenter reading and voltage
    Serial.print("Potentiometer: Reading = ");
    Serial.print(potentiomenter_reading);
    Serial.print(", Voltage = ");

    if (potentiomenter_voltage < 1.0) {
      Serial.print(potentiomenter_voltage * 1000.0, 2);
      Serial.println(" mV");
    } else {
      Serial.print(potentiomenter_voltage, 2);
      Serial.println(" V");
    }
